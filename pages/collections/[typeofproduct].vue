<template>
  <v-app>
    <v-container grid-list-xs>
      ...
      <!-- Nếu không có dữ liệu hợp lệ => Coming soon -->
      <template v-else-if="isComingSoon">
        <div class="coming-soon text-center py-16">
          <v-icon size="80" color="grey-lighten-1">mdi-clock-outline</v-icon>
          <h2 class="mt-4">Coming Soon</h2>
          <p class="mt-2">Danh mục này đang được cập nhật, vui lòng quay lại sau nhé!</p>
        </div>
      </template>

      <!-- Nếu có dữ liệu bình thường -->
      <template v-else>
        <div>
          <v-breadcrumbs style="font-size: 13px; padding: 9px 0px;" :items="items" />
          <v-img class="mx-auto" :src="bannerproduct" aspect-ratio="16/9" cover />
        </div>
          ...
          <div>
            <v-menu open-on-hover>
              <template v-slot:activator="{ props }">
                <div v-bind="props" style="border: 1px solid rgb(231, 231, 231); padding: 5px;"
              ...
              <v-list>
                <v-list-item v-for="(opt, idx) in sortOptions" :key="idx" @click="handleSort(idx)">
                  <v-list-item-title class="option-sort d-flex align-center">
                    <v-icon v-if="selectedSortIndex === idx" small color="rgb(243, 111, 64)"
        ...
        <div v-if="ortherproduct.length < store.productdata.length" class="show-more-btn my-10 text-center"
          @click="hanldeWatchmore">
          <span>
            Xem thêm những sản phẩm <strong>{{ productTitle[0].toLocaleLowerCase() + productTitle.substring(1)
              }}</strong>
          </span>
        </div>
      </template>
...
switch (typeofproduct) {
  ...
  case 'pre-cut-product':
    ...
    productType = 'CT'
    bannerproduct =
      'https://file.hstatic.net/200000377165/collection/hop_cat_thu_trai_cay_website_catergory_banner_debeabd1b2f84768b2493b5edfaabcc8.png'
    break
  case 'viet-nam-product':
    ...
    productType = 'VI'
    bannerproduct =
      'https://file.hstatic.net/200000377165/collection/banner_trai_cay_viet_2e7b9948ce92487895a56ffdbcbbfeea.png'
    break
  default:
    // Gắn Coming Soon và tắt loading
